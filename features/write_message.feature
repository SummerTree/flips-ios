Feature: Writing a reply or a new flip
  As a user
  I want to write a message to my frinds
  So, They can see my pictures, videos and words


Scenario: Showing user message when the user is replying FlipBoys conversation
  Given I am on the "View Flips" screen
  And I am watching FlipBoys message
  When I touch "Chat bubble" icon
  Then I should see a purple ballon with the message: Just for fun, lets type I love Flips

Scenario: Showing user message when the user is replying a flips didn't send by Flips
  Given I am on the "View Flips" screen
  And I am watching FlipBoys flips
  When I touch "Chat bubble" icon
...
Scenario: Writing a message to reply
  Given I am on the "View Flips" screen
  And I am seeing the text field to reply a flips
